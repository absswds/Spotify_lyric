...
- **2026-08** — オフラインモードを追加：Spotify App Remote が接続できない（ネットワークなし）場合、通知リスナー経由で Spotify クライアント自身のシステム MediaSession（曲・アートワーク・再生コントロール）を読み取り、LRCLIB キャッシュの歌詞を表示します。デフォルトの歌詞ソースを LRCLIB（最もクリーンな LRC）に変更。網易雲と QQ 音楽はセッション内のみとなり、オフラインキャッシュには LRCLIB 歌詞のみが保存されます。
...
> **オフラインモード**：初回起動時にシステムから**通知アクセス**（通知使用権）の許可を求められる場合があります。この権限は、ネットワークがない状態（機内モード / Spotify オフライン再生）で Spotify のメディアセッションから現在の曲を読み取るために必要です。キャッシュ済みの歌詞・アートワーク・再生コントロールがオフラインでも動作します。一度だけ許可してください — アプリはメディアセッションのデータのみを読み取り、通知内容は読み取りません。
...
> 歌詞画面が空白になった場合（Spotify ネイティブプレーヤーが前面に出た場合）、または背景から戻った後に歌詞が更新されない場合、最近のタスクからアプリをスワイプして閉じ、再度開くと復元します。
