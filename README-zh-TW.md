...
- **2026-08** — 新增離線模式：當 Spotify App Remote 無法連線（無網路）時，應用程式透過通知監聽器讀取 Spotify 用戶端自身的系統 MediaSession（曲目、封面、播放控制），並顯示 LRCLIB 快取歌詞。預設歌詞來源改為 LRCLIB（格式最乾淨）；網易雲與 QQ 音樂改為僅工作階段內使用，離線快取只保留 LRCLIB 歌詞。
...
> **離線模式**：首次開啟應用程式時，系統可能提示授予**通知使用權**。此權限用於在無網路（飛航模式 / Spotify 離線播放）時讀取 Spotify 自身的媒體工作階段，以便繼續顯示快取歌詞、封面並支援播放控制。只需授予一次——應用程式只讀取媒體工作階段資料，不會讀取通知內容。
4. 在 Spotify 播放任意歌曲。
