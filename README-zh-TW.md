...
2. 點擊 **Create App**。
3. 在 **Redirect URIs** 加入且必須完全一致的：`spotifylyricsproxy://callback`。
4. 勾選 **Web API** 後儲存。
5. 複製頁面頂部的 **Client ID**。

> Client ID 不是 client secret，但仍不要將你的個人設定檔或 token 提交到公開倉庫。
...
本專案僅提供在使用者裝置上搜尋、顯示與快取的介面；倉庫不包含歌詞資料庫、歌曲 metadata dump、封面檔案庫或代管歌詞 API。
