...
- **2026-08** — 新增离线模式：当 Spotify App Remote 无法连接（无网络）时，应用通过通知监听器读取 Spotify 客户端自身的系统 MediaSession（曲目、封面、播放控制），并显示 LRCLIB 缓存歌词。默认歌词源改为 LRCLIB（格式最干净）；网易云与 QQ 音乐改为仅会话内使用，离线缓存只保留 LRCLIB 歌词。
...
> **离线模式**：首次打开应用时，系统可能提示授予**通知使用权**。该权限用于在无网络（飞行模式 / Spotify 离线播放）时读取 Spotify 自身的媒体会话，从而继续显示缓存歌词、封面并支持播放控制。只需授予一次——应用只读取媒体会话数据，不读取通知内容。
...
> 如果 App 播放页面变成空白（Spotify 原生播放器抢占了画面），或在后台回来后歌词没有及时更新，在最近任务中滑掉 App 重新打开即可恢复。
...
| 离线（飞行模式）时无歌词 | 未授予通知使用权 | 在系统设置中开启通知使用权；离线只能显示在线时缓存过的 LRCLIB 歌词 |
